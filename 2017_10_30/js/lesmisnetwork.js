function Network() {
  ...
  function findDegrees(nodes, links) {
    nodes.forEach(function(n) {
      n.degree = links.map(function(l) {
        return l.target == n.id || l.source == n.id ? 1 : 0
      }).reduce((a,b) => a+b, 0)
    })
  }
  ...
  function setupData(data) {
    ...
    findDegrees(data.nodes, data.links)
  ...
  function showDetails(d, i) {
    var content;
    content = '<p class="main"> Character: ' + d.id + '</span></p>'
    content += '<hr class="tooltip-hr">';
    content += '<p class="main">Number of interactions: ' + d.degree + '</span></p>'
  ...
  function updateNodes() {
    ...
    node.enter()
        .append("circle")
        .attr("class", "node").attr("cx", function(d) {return d.x})
  ...
  network = function(selection, data) {
    ...
    force.on("tick", forceTick).charge(-480).linkDistance(50)
